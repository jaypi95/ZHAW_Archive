...
n_max = 200  # Maximale Anzahl Iterationen
eps = 1e-4  # Maximaler Fehler

# Wähle das Abbruchkriterium:
abbruchkriterium = "absolute_error"  # "max_iterations", "relative_error", "absolute_error", "max_residual"
...
def is_finished_absolute_error(f, x, eps):
    if x.shape[0] < 2:
        return False
    c = 1
    x1_exact = (4 * c - np.sqrt(9 * c ** 2 - 7))/ (c ** 2 + 1)
    x2_exact = ((c * np.sqrt(9 * c ** 2 - 7) - 4 * c**2) / (c ** 2 + 1)) + 4
    if(np.abs(x1_exact - x[-1][0]) <= eps and np.abs(x2_exact - x[-1][1]) <= eps):
        return True
    else:
        return False
