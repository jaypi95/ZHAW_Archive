...
# fixme this maybe broken fix it silvan
...
while not is_finished(f_lambda, x_approx):
    ...
    print(
        f'Berechne das Residuum der letzten Iteration ‖f(x(n))‖₂ = √({str(f_lambda(x_n)[0][0])} + {str(f_lambda(x_n)[1][0])}) -> {str(last_residual)}')
    ...
    while k <= k_max:
        ...
        print(f'Berechne das neue Residuum mit k = {str(k)} ‖f(x(n) + 𝛅(n) / 2^k)‖₂ = {str(new_residual)}')
